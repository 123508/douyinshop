...
create table if not exists order_status_log(
     id  binary(16) not null default UUID_TO_BIN(uuid(),1) comment '订单日志id',
    ...
    -- 防止重复状态变更 (相同订单+新状态+时间戳去重)
     unique index udx_order_status (order_id, new_status, (UNIX_TIMESTAMP(event_time) div 10)),

     constraint chk_status check ((old_status between 0 and 9) and (new_status between 0 and 9))
